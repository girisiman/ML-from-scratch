...
def classification(alphas, target, kernel, X_train, x_test, b):
...
if __name__ == '__main__':
    data1 = pd.read_csv("./01_train.csv")
    ...
    features_scaled = scaler.fit_transform(features, target)
    C = 100.0
    m = len(features)
    ...
    # Instantiate model
    model = SMOMODEL(features_scaled, target, C, gaussian_kernel,
    ...
    data2 = pd.read_csv('./01_test.csv')
    data_test = np.asarray(data2)
    features_test = data_test[:, :-1]
    target_test = data_test[:, -1]
    #print(features_test)
    cdf = classification(model.alphas,model.y,model.kernel,model.X,features_test,model.b)
